...
export async function POST(req: NextRequest) {
  ...
  // ─── Rate limiting: max 5 registrations/min per IP ───
  const ip = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || req.headers.get("x-real-ip") || "unknown";
  const rl = checkRateLimit(`register:${ip}`, REGISTER_RATE_LIMIT);
  if (!rl.allowed) {
    return json({ success: false, message: `Demasiadas solicitudes. Intenta de nuevo en ${Math.ceil(rl.retryAfterSec / 60)} minutos.` }, 429);
  }
