...
export default function ManageAppsPage() {
  ...
  const [currentApp, setCurrentApp] = useState<AppData>(appsList[0]);
  ...
  const handleCreateApp = (e: React.FormEvent) => {
    ...
    setFeedback("Nueva aplicación creada con éxito.");
  ...
  const getCode = (lang: string) => {
    if (lang === "csharp") {
      return `using RealAuthX;\nvar app = new Client("${currentApp.name}", "${currentApp.ownerId}", "${currentApp.secret}", "${currentApp.version}");\nawait app.InitAsync();`;
    }
    if (lang === "cpp") {
      return `#include "RealAuth.h"\nRealAuth::Client AuthOwner(RealAuth::ApiUrl(), "${currentApp.name}", "${currentApp.ownerId}", "${currentApp.secret}", "${currentApp.version}");\nAuthOwner.Init();`;
    }
    if (lang === "python") {
      return `from realauthx import RealAuth\napp = RealAuth("${currentApp.name}", "${currentApp.ownerId}", "${currentApp.secret}", "${currentApp.version}")\napp.init()`;
    }
    return `const RealAuth = require('realauthx');\nconst app = new RealAuth("${currentApp.name}", "${currentApp.ownerId}", "${currentApp.secret}", "${currentApp.version}");\nawait app.init();`;
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in text-slate-200">
      ...
      {/* ── TOP APP HEADER CARD (MATCHING SCREENSHOT 3) ── */}
      <div className="rounded-2xl bg-[#040e24]/85 border border-[#0099ff]/25 p-5 backdrop-blur-2xl shadow-xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">App RealAuthX</p>
          </div>
        </div>
      ...
      {/* ── CARD 1: INICIALIZACION DE LA APLICACION (MATCHING SCREENSHOT 3) ── */}
      ...
      {/* ── CARD 3: CLAVES DE ACCESO & ACCIONES (MATCHING SCREENSHOTS 3 & 4) ── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Claves de acceso */}
      ...
      <div className="rounded-2xl bg-[#040e24]/85 border border-[#0099ff]/25 p-6 backdrop-blur-2xl shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-gradient-to-r from-[#0080ff] to-[#00b4ff] hover:from-[#0070e0] hover:to-[#00a2ff] text-white font-extrabold text-xs rounded-xl shadow-[0_0_20px_rgba(0,153,255,0.4)] transition-all cursor-pointer whitespace-nowrap"
          >
            Crear aplicacion
          </button>
        </div>
      ...
      {/* ── CARD 6: MIS APLICACIONES TABLA (MATCHING SCREENSHOT 4) ── */}
      <div className="rounded-2xl bg-[#040e24]/85 border border-[#0099ff]/25 p-6 backdrop-blur-2xl shadow-2xl space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono mt-0.5">{appsList.length} aplicación(es)</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {appsList.map((app) => (
                <tr key={app.id} className="hover:bg-[#071738]/40 transition-colors">
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">App RealAuthX</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-3.5 px-3 text-right">
                    <div className="flex items-center justify-end gap-1.5 font-sans">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-[#07193b] hover:bg-[#0c2452] border border-[#0099ff]/30 text-[11px] font-bold text-slate-200"
                      >
                        Copiar todo
                      </button>
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-[#0088ff] hover:bg-[#0099ff] text-white text-[11px] font-bold"
                      >
                        Seleccionar
                      </button>
                      <button
                      ...
                      {appsList.length > 1 && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg bg-rose-950/30 hover:bg-rose-950/50 border border-rose-500/40 text-[11px] font-bold text-rose-400"
                        >
                          Eliminar
                        </button>
                      )}
      ...
      {/* ── MODAL: CREAR NUEVA APLICACION (MATCHING SCREENSHOT 1 DE TURNO 2) ── */}
      {createModalOpen && (
            ...
            <form onSubmit={handleCreateApp} className="space-y-4 pt-1">
              <div>
                <input
                  ...
                  required
                  className="w-full bg-[#020713] border border-[#0099ff]/30 focus:border-[#00c2ff] rounded-xl px-4 py-3 text-xs text-white placeholder:text-slate-500 focus:outline-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-gradient-to-r from-[#0080ff] to-[#00b4ff] hover:from-[#0070e0] hover:to-[#00a2ff] text-white font-extrabold text-xs rounded-xl shadow-lg transition-all cursor-pointer"
                >
                  Crear
                </button>
              </div>
