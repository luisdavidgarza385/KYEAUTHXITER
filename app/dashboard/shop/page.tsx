...
import { Coins, CreditCard, Sparkles, Check, Loader2, ArrowRight, X, MessageSquare, ExternalLink } from "lucide-react";
...
export default function ShopPage() {
  ...
  const plans = [
    ...
    },
    {
      ...
      price: 7.00,
      period: "año",
      description: "Ahorra más del 70% y asegura tu plataforma todo el año.",
      features: [
        "Licencias Ilimitadas",
        "Usuarios Ilimitados",
        "Panel sin anuncios ni marcas",
  ...
  useEffect(() => {
    ...
    async function initPaypal() {
      ...
      try {
        ...
        if (!res.ok || !data.clientId || data.clientId === "demo_paypal_client_id") {
          throw new Error("Cliente de PayPal no configurado");
        }
  ...
  // Effect to render PayPal Buttons when SDK is ready
  useEffect(() => {
    if (!sdkReady || !showPaypalModal || !selectedPlan || hasPaypalError) return;
    ...
    try {
      ...
      console.error("Fallo al renderizar botones:", err);
    }
  }, [sdkReady, showPaypalModal, selectedPlan, hasPaypalError]);
    ...
    <div className="p-6 lg:p-8 max-w-[1200px] mx-auto space-y-8 text-zinc-350">
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        {plans.map((p) => (
          <div
            key={p.name}
            className={`relative flex flex-col justify-between rounded-2xl border bg-zinc-950/40 p-8 shadow-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-sky-500/5 ${
              p.popular
                ? "border-sky-500 bg-gradient-to-b from-sky-950/10 to-transparent"
                : "border-zinc-800"
            }`}
          >
            {p.popular && (
              <span className="absolute -top-3.5 right-6 rounded-full bg-sky-500 px-3 py-1 text-[10px] font-black uppercase text-zinc-950 tracking-wider">
                Más Popular / Ahorro
              </span>
            )}
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black text-zinc-150 font-mono">${p.price.toFixed(2)}</span>
              ...
              <ul className="space-y-3">
                {p.features.map((f) => (
                  <li key={f} className="flex items-center gap-2.5 text-xs text-zinc-400">
                    <div className="rounded-full bg-sky-500/10 p-0.5 text-sky-400 shrink-0">
      ...
      {showPaypalModal && selectedPlan && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#0b0f19] border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-6 text-zinc-100 relative">
            {/* Header */}
            ...
            <div className="bg-zinc-950/50 border border-zinc-850 p-4 rounded-xl space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-bold text-lg text-sky-400">${selectedPlan.price.toFixed(2)} USD</span>
              </div>
            </div>

            {/* Loader / PayPal Container / Error Fallback */}
            <div className="space-y-4">
              {sdkLoading && (
                <div className="flex flex-col items-center justify-center py-8 space-y-3">
                  <Loader2 className="w-8 h-8 text-sky-400 animate-spin" />
                  <span className="text-xs text-zinc-400">Cargando pasarela de pago segura...</span>
                </div>
              )}
              ...
              {/* Direct Admin Contact fallback if PayPal Client ID is missing */}
              {hasPaypalError && (
                <div className="bg-sky-950/20 border border-sky-500/30 p-5 rounded-xl space-y-4 text-center">
                  <div className="w-10 h-10 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center mx-auto">
                    <MessageSquare className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-white">Pago Directo con el Administrador</h4>
                    <p className="text-xs text-zinc-400 mt-1 leading-relaxed">
                      La pasarela automática está recibiendo mantenimiento. Para activar tu **{selectedPlan.name}** (${selectedPlan.price.toFixed(2)} USD) de forma inmediata, contacta al desarrollador principal.
                    </p>
                  </div>
                  
                  <div className="pt-2 space-y-2">
                    <a
                      href="https://wa.me/?text=Hola%2C%20quiero%20comprar%20el%20"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider transition"
                    >
                      Contactar por WhatsApp <ExternalLink className="w-3.5 h-3.5" />
                    </a>
                  </div>
                </div>
              )}
              ...
              {loading === "paypal_capture" && (
                <div className="bg-sky-950/20 border border-sky-900/30 rounded-lg p-3.5 text-xs text-sky-400 flex items-center gap-2">
