...
export default function SellerAPIPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {/* Web Store Link - NEW */}
      <div className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 border border-blue-700/50 rounded-lg p-6 mb-6">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-lg bg-blue-500/20 border border-blue-500/30 flex items-center justify-center">
            <ExternalLink className="w-5 h-5 text-blue-400" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-white">Tienda Web Pública</h2>
            <p className="text-sm text-gray-400">Enlace para que tus clientes generen licencias</p>
          </div>
        </div>

        <div className="flex items-center gap-2 mb-4">
          <code className="flex-1 bg-gray-900 text-blue-300 text-sm px-4 py-3 rounded font-mono break-all">
            {baseUrl}/store/{seller.seller_key}
          </code>
          <button
            onClick={() => copyToClipboard(`${baseUrl}/store/${seller.seller_key}`, 'storeurl')}
            className="px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded transition flex items-center gap-2"
          >
            {copied === 'storeurl' ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
            {copied === 'storeurl' ? 'Copiado' : 'Copiar'}
          </button>
        </div>

        <a
          href={`/store/${seller.seller_key}`}
          target="_blank"
        ...
        <div className="mt-4 text-xs text-gray-400">
          💡 <strong>Tip:</strong> Comparte este enlace con tus clientes. Ellos podrán generar sus propias licencias usando un formulario web.
        </div>
      </div>
