...
export default function SubscriberDashboard() {
  ...
  const checkAuth = async () => {
    try {
      const response = await fetch("/api/subscriber/me");
  ...
  const loadStats = async () => {
    try {
      const response = await fetch("/api/subscriber/stats");
      const data = await response.json();
  ...
  const isUnlimited = stats.credits === -1;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mt-8 bg-blue-900/20 border border-blue-500/30 rounded-lg p-6">
          <h3 className="text-lg font-bold text-blue-300 mb-2">
            💡 Tu Suscripción: {stats.subscriptionType}
          </h3>
          <ul className="text-gray-300 space-y-2 text-sm">
            <li>✅ Crea aplicaciones ilimitadas</li>
            <li>✅ Genera licencias sin límite</li>
            <li>✅ Gestiona usuarios y suscripciones</li>
